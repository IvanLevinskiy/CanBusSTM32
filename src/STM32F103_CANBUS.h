#ifndef STM32F103CANBUS_H_
#define STM32F103CANBUS_H_
...
/* 
    СИМВОЛЬНОЕ НАИМЕНОВАНИЕ BITRATE ДЛЯ CANBUS                         
*/
typedef enum CANBUS_BITRATE 
{
    CAN_5KBPS   = 5000,
    CAN_10KBPS  = 10000,
    CAN_20KBPS  = 20000,
...
/* 
    СИМВОЛЬНОЕ ИМЯ ФОРМАТА СООБЩЕНИЯ CANBUS                                
*/
typedef enum CAN_FORMAT
{
  STANDARD_FORMAT = 0, 
  EXTENDED_FORMAT
};
...
/* 
    ТИП СООБЩЕНИЯ CANBUS
*/
typedef enum CAN_FRAME
{
    DATA_FRAME = 0, 
    REMOTE_FRAME
};
...
/*
    КОНФИГУРАЦИЯ ТАЙМИНГОВ CANBUS
*/
typedef struct CAN_bit_timing_config_t
{
...
/* 
    ПЕРЕОПРЕДЕЛЕНИЕ ПИНОВ ДЛЯ CANBUS
 */
typedef enum CANBUS_PINS_MAPPED
{
  CANBUS_PINS_MAPPED_RX_PA11_TX_PA12 = 0, 
  CANBUS_PINS_MAPPED_RX_PB8_TX_PB9 = 2, 
  CANBUS_PINS_MAPPED_RX_PD0_TX_PD1 = 3
};
 ...
 */
int16_t ComputeCANTimings(const uint32_t peripheral_clock_rate, const uint32_t target_bitrate, CAN_bit_timing_config_t* const out_timings)
{
    ...
    if (DEBUG) 
    {
      Serial2.print("target_bitrate=");
      Serial2.println(target_bitrate);
      Serial2.print("peripheral_clock_rate=");
      Serial2.println(peripheral_clock_rate);
  
      Serial2.print("timings.baud_rate_prescaler=");
      Serial2.println(out_timings->baud_rate_prescaler);
      Serial2.print("timings.time_segment_1=");
      Serial2.println(out_timings->time_segment_1);
      Serial2.print("timings.time_segment_2=");
      Serial2.println(out_timings->time_segment_2);
      Serial2.print("timings.resynchronization_jump_width=");
      Serial2.println(out_timings->resynchronization_jump_width);
    }
    return 0;
}
 ...
 *
 */
bool CanBusInitialize(CANBUS_BITRATE bitrate, CANBUS_PINS_MAPPED remap)
{
  // Reference manual
 ...
 * 
 */
void CanBusReadMsg(CAN_msg_t* CAN_rx_msg)
{
...
void CANSend(CAN_msg_t* CAN_tx_msg)
{
  ...
  if (CAN1->sTxMailBox[0].TIR & 0x1UL) {
    Serial2.println("Send Fail");
    Serial2.println(CAN1->ESR);
    Serial2.println(CAN1->MSR);
    Serial2.println(CAN1->TSR);
  }
}
 ...
 *
 */
uint8_t CanBusIsAvailable(void)
{
